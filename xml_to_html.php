...
{
	$format = '<a class="btn btn-default" style="margin-bottom:8px;" href="%unescapedUrl%">%visibleUrl%</a>&nbsp;';
	
	
	$xml = simplexml_load_file('json_to_xml.xml');
	$result = $xml->xpath('/myroot/myelement');
...
function getVideoobject()
{
	...
	$xml = simplexml_load_file('json_to_xml.xml');
	$result = $xml->xpath('/myroot/myelement/videoobject');
